...
public class EmpController {
	...
	public String bookInsert(Model model) {
		BookVO bookVO = service.getBookNo();
		model.addAttribute("bookVO", bookVO);
		return "book/bookInsert";
	}
	// 도서정보 등록 >> 처리
	@PostMapping("bookInsert")
	public String bookInsertProc(BookVO bookVO) {
		int bno = service.addBook(bookVO);
		String uri = null;
		if(bno > -1) {
			uri = "redirect:bookList";
		}
		return uri;
	}
	// 도서목록
	...
	public String rentList(Model model) {
		List<RentVO> list = service.rentList();
